...
`Session` is a branded type you cannot construct or read in application code, and the runtime backs that up. Each adapter creates a session box (`createSessionBox`) inside its factory; a sealed session is a frozen object whose substrate value lives in the box's private WeakMap. `JSON.stringify(session)` returns `"[upact:session]"` (also when nested in a larger object). `Object.keys` gives `[]`, `Reflect.ownKeys` gives only `['toJSON']`, property reads return `undefined`, and `util.inspect` and `structuredClone` leak nothing. So a session accidentally logged, serialised into a response, or posted across a worker boundary carries no tokens. The only thing an app can do with a `Session` is hand it back to the port (for example to `invalidate`).

The adapter that sealed a session must be able to open it again: when the app calls `invalidate(session)`, the adapter needs the token inside to revoke it, and the box's `unseal` is how it gets it back. This is adapter code; the box factory lives in the core package only so that every adapter shares one audited seal-and-unseal implementation instead of reimplementing it. It is exported from the `@prefig/upact/internal` subpath, which keeps it out of application imports, and importing it gains an application nothing: a fresh box can unseal no existing session, and the populated box never leaves the adapter's closure. A session is meaningful only to the adapter instance that created it: a clone, a restarted process, or another instance's session all unseal to `undefined`.
