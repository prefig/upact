...
Wrap substrate session values with `createSession` from `@prefig/upact` and recover them only with `_unwrapSession` from `@prefig/upact/internal` (see `upact/src/runtime.ts`). Do not roll your own opaque wrapper; the core one is frozen, WeakMap-backed and serialises to `"[upact:session]"`. Keep unwrap sites few; ember marks its single call site "The ONLY `_unwrapSession` site in this package" (`upact-ember/src/adapter.ts`).
