...
Report security issues by email to **fiore@prefig.tech**. Do not open a public issue for a security vulnerability.
...
- **Session-association leak.** A pattern that makes an adapter's session-to-state association (its closure-held WeakMap) reachable outside the adapter's closure — through an export, an instance property, an error object, a debug hook, or any other channel application code can reach.
...
- Social-engineering attacks against adapter maintainers. These are supply-chain concerns. Adapter repositories that ship their own security policy handle them there; adapters without one fall back to this policy's reporting address.
