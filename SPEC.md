...
- **Upactor** — a `Upactor` value as defined in §4. The application's view of "who is this." (Renamed from `UserIdentity` in v0.1; the deprecated alias was removed in v0.2.)
...
`Session` is a request-scoped value: an application MUST NOT assume a `Session` obtained on one request is recognised on a later one. `invalidate` MUST degrade safely when the association is missing — a documented no-op or a request-derived fallback (cookies, a per-request client). Adapters whose substrate holds revocable server-side state SHOULD derive invalidation state from the request and treat any session-keyed lookup as a same-request optimisation; an adapter that cannot MUST document the consequence in its conformance statement.
