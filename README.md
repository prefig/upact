...
The escape hatch is `_unwrapSession`, exported only from the `@prefig/upact/internal` subpath. The adapter that sealed a session must be able to open it again: when the app calls `invalidate(session)`, the adapter needs the token inside to revoke it, and `_unwrapSession` is how it gets it back. This is adapter code; it lives in the core package only so that every adapter shares one audited seal-and-unwrap pair instead of reimplementing it. The `/internal` path keeps it out of application imports. Unwrapping is process-local: a restarted process or a cloned session yields `undefined`.
