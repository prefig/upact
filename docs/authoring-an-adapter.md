...
Create one session box per adapter instance with `createSessionBox` from `@prefig/upact/internal` (see `upact/src/runtime.ts`), as the first line of your factory; seal with `box.seal`, recover with `box.unseal`, and never let `unseal` leave the closure. Do not roll your own opaque wrapper; the core one is frozen, WeakMap-backed and serialises to `"[upact:session]"`. A session is meaningful only to the adapter instance that sealed it; `unseal` returns `undefined` for everything else, so if your adapter may seal `null`, check `=== undefined`, not truthiness. Keep unseal sites few; ember marks its single call site (`upact-ember/src/adapter.ts`).
